...
# Perplexity API key
api_key = "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"
...
class DataProcessor:
    ...
    def get_ratio_value(self, target_id):
        ...
        if result is None:
            print(f"Value not found for {target_id}")
            return None
        try:
            return float(result)
        except ValueError:
    ...
    def process_company_overview(self):
        try:
            profile_data = self.raw_data['US7731221062']['profile']['data']
            contacts = profile_data['contacts']
            self.processed_data['company_overview'] = {
                'legal_name': contacts.get('NAME', 'N/A'),
                'sector': profile_data.get('sector', 'N/A'),
                'industry': profile_data.get('industry', 'N/A'),
                'description': profile_data.get('businessSummary', 'N/A'),
                'address': contacts.get('ADDRESS', 'N/A'),
                'city': contacts.get('CITY', 'N/A'),
                'state': contacts.get('STATEREGION', 'N/A'),
                'country': contacts.get('COUNTRY', 'N/A'),
                'postal_code': contacts.get('POSTCODE', 'N/A'),
                'phone': contacts.get('TELEPHONE', 'N/A'),
        ...
        try:
            ...
            data = {
                ...
                "messages": [{
                    "role": "user",
                    "content": (
                        f"Conduct a detailed SWOT analysis for {company_name}. "
                        "Provide specific points for Strengths, Weaknesses, Opportunities, and Threats. "
                        "Format the response as a JSON object with keys 'Strengths', 'Weaknesses', 'Opportunities', and 'Threats', "
                        "each containing an array of concise points."
                    )
                }],
                "max_tokens": 1000
            }
            ...
            print(f"Unexpected error in API handling: {e}")
        return None
...
class PDFGenerator:
    ...
    def generate_table(self, title, data, columns):
        try:
            ...
            print(f"Error generating table {title}: {e}")
    ...
    def generate_company_overview(self):
        try:
            overview = self.processed_data['company_overview']
            self.story.append(Paragraph("Company Overview", self.styles['Heading1']))
            for key, value in overview.items():
                self.story.append(Paragraph(
                    f"<b>{key.replace('_', ' ').title()}:</b> <font size='+1'>{value}</font>",
                    self.styles['Normal']
                ))
                self.story.append(Spacer(1, 6))
            self.story.append(Spacer(1, 12))
        except KeyError as e:
            print(f"Error generating company overview: Missing key {e}")
    ...
    def generate_financial_snapshot(self):
        try:
            ...
            print(f"Error generating financial snapshot: Missing key {e}")
    ...
    def generate_valuation_analysis(self):
        try:
            ...
            print(f"Error generating valuation analysis: Missing key {e}")
    ...
    def generate_efficiency_and_profitability(self):
        try:
            ...
            print(f"Error generating efficiency and profitability: Missing key {e}")
